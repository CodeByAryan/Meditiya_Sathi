...
type Props = { targetAt: string; className?: string };
type Parts = { days: number; hours: number; minutes: number; seconds: number; state: "UPCOMING" | "LIVE" | "COMPLETED" };

function remaining(targetAt: string): Parts {
  const target = new Date(targetAt).getTime();
  const ms = target - Date.now();
  if (!Number.isFinite(target)) return { days: 0, hours: 0, minutes: 0, seconds: 0, state: "COMPLETED" };
...
export default function FestivalCountdown({ targetAt, className = "" }: Props) {
  const [parts, setParts] = useState(() => remaining(targetAt));
  useEffect(() => { setParts(remaining(targetAt)); const id = window.setInterval(() => setParts(remaining(targetAt)), 1000); return () => window.clearInterval(id); }, [targetAt]);
